...
class ventanaEtiquetar:
    ...
    def __init__(self):
        ...
        left_col = [
            [sg.Text('Bienvenido a Etiquetar Imagenes', font=('Arial', 12))],
            [sg.Text('Folder'), sg.In(size=(25, 1), background_color='white', enable_events=True, key='-FOLDER-'), sg.FolderBrowse()],
            [sg.Listbox(values=[], enable_events=True, size=(40, 20), key='-FILE LIST-')],
            [sg.Text('Tags')],
            [sg.Input(key='-TAGS-', background_color='white'),
             sg.Button('Agregar', key='AgregarTags')],
            [sg.Text('Descripción')],
            [sg.Input(key='-DESC-', background_color='white'),
             sg.Button('Agregar', key='AgregarDesc')]
        ]

        right_col = [
            [sg.Button('Volver', size=(10, 2), pad=((280, 0), (0, 0)))],
            [sg.Text(size=(40, 1), background_color='white', key='-TOUT-')],
            [sg.Image(background_color='white', key='-IMAGE-', size=(340, 340))],
            [sg.Text('Tamaño:', background_color='white', key='-TAMAÑO-')],
            [sg.Text('Tipo:', background_color='white', key='-TIPO-')],
            [sg.Text('Resolucion:', background_color='white', key=('-RESOLUCION-'))],
            [sg.Text("", background_color='white', key="-TAGVALUE-")],
            [sg.Text("", background_color='white', key="-DESCVALUE-")],
            [sg.Push(background_color='white'), sg.Button('Guardar', key='-GUARDAR-', size=(10, 2))]
        ]

        layout = [[sg.Column(left_col, background_color='white'), sg.Column(right_col, background_color='white')]]

        # Crea la ventana con el layout
        self.window = sg.Window('UNLPImage', layout, size=(800, 600), background_color='white')
    ...
    def iniciar_ventana(self, perfil_act):
        ...
        while True:
            ...
            if event == '-GUARDAR-':
                ...
                sg.popup_no_border('Se guardo correctamente la metadata de la imagen', button_color='black', background_color='white', text_color='black', )
